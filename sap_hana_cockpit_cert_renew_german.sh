...
log "Kontext: SID=${SID} INSTANCE=${INSTANCE} FQDN=${FQDN} API=${XSA_API} conf=${SITE_CONF_LOADED:-keine}"
...
if [ "$CERT_STATE" -gt 0 ]; then
    ...
    write_spool 1 "Zertifikat aktiv am Router-Port ${VERIFY_PORT} | expires: ${EXPIRY}${DAYS_TXT}${DEPLOY_TXT}${ACME_DRIFT}" "$PERF"
else
